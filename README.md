...
Every time the charger connects to the Home Assistant WebSocket server, the integration automatically enforces the following configuration options to ensure a reliable handshake and consistent telemetry:

- TxBeforeAcceptedEnabled: Set to true.
- AuthorizeRemoteTxRequests: Set to false.
- UnlockConnectorOnEVSideDisconnect: Set to false.
- MeterValueSampleInterval: Set to 30 seconds.
- MeterValuesSampledData: Enforces a full set of diagnostic measurands including:
    - Energy, Power (Total & Per Phase), Voltage, Current (Import & Offered), Reactive Power, Power Factor, Frequency, and Temperature.
...
- Dynamic Phasing: Automatically detects if the charger is a Single-Phase (7kW) or 3-Phase (22kW) model and adjusts the charging schedules accordingly.
...
- Voltage (V) - Per phase.
- Current (A) - Per phase.
- Current Offered (A) - The limit advertised to the car.
...
### Charger Settings
Log into your SEVR Opti via the Ecosolex Connect application and set the OCPP Central System URL to:
